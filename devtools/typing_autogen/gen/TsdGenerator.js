...
function TsdGenerator() {
  var gSobject = gs.init('TsdGenerator');
  gSobject.clazz = { name: 'TsdGenerator', simpleName: 'TsdGenerator'};
  gSobject.clazz.superclass = { name: 'java.lang.Object', simpleName: 'Object'};
  gSobject.config = null;
  gSobject.events = null;
  gSobject.schema = null;
  gSobject['generate'] = function(it) {
    gSobject.events = gs.mc(gSobject,"readPlotlyEvents",[]);
    return gs.mc(gSobject,"readJson",[]);
  }
  gSobject['readJson'] = function(it) {
    var slurper = JsonSlurper();
    gSobject.schema = gs.mc(slurper,"parseText",[gs.gp(gs.gp(gSobject.config,"jsonSchema"),"text")]);
    var schemaParser = SchemaParser(gs.map().add("schema",gSobject.schema));
    gs.mc(schemaParser,"parseSchema",[]);
    var tsdWriter = TsdWriter(gs.map().add("config",gSobject.config).add("schemaParser",schemaParser).add("events",gSobject.events));
    gs.mc(tsdWriter,"writeTsd",[]);
    return gSobject.schema;
  }
  gSobject['readPlotlyEvents'] = function(it) {
    var events = gs.map();
    gs.sp(events,"dataEventNames",gs.list(["plotly_click" , "plotly_hover" , "plotly_unhover" , "plotly_selecting" , "plotly_selected"]));
    gs.sp(events,"updateEventNames",gs.list(["plotly_restyle" , "plotly_relayout"]));
    gs.sp(events,"allEvents",gs.list([]));
    gs.sp(events,"dataEvents",gs.list([]));
    gs.sp(events,"updateEvents",gs.list([]));
    gs.sp(events,"otherEvents",gs.list([]));
    var matcher = null;
    gs.mc(gs.gp(gSobject.config,"jsSourceFile"),"eachLine",[function(line) {
      if (matcher = (gs.regExp(line,/(emit\(')([^']+)'/))) {
        return gs.mc(gs.gp(events,"allEvents"),'leftShift', gs.list([((matcher[0])[2])]));
      } else {
        if (matcher = (gs.regExp(line,/(triggerHandler\(.*')([^']+)'/))) {
          return gs.mc(gs.gp(events,"allEvents"),'leftShift', gs.list([((matcher[0])[2])]));
        };
      };
    }]);
    gs.sp(events,"allEvents",gs.mc(gs.gp(events,"allEvents"),"unique",[]));
    gs.mc(gs.gp(events,"allEvents"),"each",[function(thisEvent) {
      if (gs.mc(gs.gp(events,"dataEventNames"),"contains",[thisEvent])) {
        return gs.mc(gs.gp(events,"dataEvents"),'leftShift', gs.list([thisEvent]));
      } else {
        if (gs.mc(gs.gp(events,"updateEventNames"),"contains",[thisEvent])) {
          return gs.mc(gs.gp(events,"updateEvents"),'leftShift', gs.list([thisEvent]));
        } else {
          return gs.mc(gs.gp(events,"otherEvents"),'leftShift', gs.list([thisEvent]));
        };
      };
    }]);
    gs.println(" ");
    gs.println("Events found in plotly.js source file. Manually copy into plotly-events.model.ts:");
    gs.println(" ");
    gs.println("/**");
    gs.println(" * Plotly events that typically handle event data of type {CartesianEventPoint}.");
    gs.println(" */");
    gs.println("export class PlotlyDataEvents {");
    gs.mc(gs.gp(events,"dataEvents"),"each",[function(thisEvent) {
      return gs.println("  static " + (thisEvent) + " = \"" + (thisEvent) + "\";");
    }]);
    gs.println("}");
    gs.println(" ");
    gs.println("/**");
    gs.println(" * Plotly update events.");
    gs.println(" * Event data includes an array containing an object of updated attributes and an array of the trace numbers that were updated.");
    gs.println(" */");
    gs.println("export class PlotlyUpdateEvents {");
    gs.mc(gs.gp(events,"updateEvents"),"each",[function(thisEvent) {
      return gs.println("  static " + (thisEvent) + " = \"" + (thisEvent) + "\";");
    }]);
    gs.println("}");
    gs.println(" ");
    gs.println("/**");
    gs.println(" * Other Plotly events that either have no event data, or have custom event data (see Plotly docs or source code).");
    gs.println(" */");
    gs.println("export class PlotlySimpleEvents {");
    gs.mc(gs.gp(events,"otherEvents"),"each",[function(thisEvent) {
      return gs.println("  static " + (thisEvent) + " = \"" + (thisEvent) + "\";");
    }]);
    gs.println("}");
